...
function my_archive_title($title) {
    ...
    } else if (is_tag()) { //タグアーカイブの場合
        $title = single_tag_title( '', false );
    } else if(is_post_type_archive()) { //投稿タイプのアーカイブの場合
        $title = post_type_archive_title( '', false );
    } else if (is_tax()) { //タームアーカイブの場合
        $title = single_term_title( '', false );
    } else if(is_author()) { //作者アーカイブの場合
        $title = get_the_author();
    } else if(is_date()) { //日付アーカイブの場合
        $title = '';
...
function get_post_views($id=0) {
    ...
    $count = get_post_meta($id, $count_key, true); //記事のメタ情報を取得
...
function set_post_views() {
    ...
    if($count === '') {
        ...
        if(!is_user_logged_in()) { //管理者以外の閲覧を除外
            $count++;
...
function my_posts_search($search, $wp_query) {
    //検索結果ページ・メインクエリ・管理画面以外の3つの条件がそろった場合
...
function highlight_shortcode($atts, $content) {
    ...
    return "<pre><code class='".$atts['lang']."'>".$content."\n</code></pre>";
